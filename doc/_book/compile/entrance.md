...
`compile` 函数在执行 `createCompileToFunctionFn` 的时候作为参数传入，实际上就是 `createCompiler` 函数中定义的 `compile` 函数。
...
编译入口逻辑之所以这么绕，是因为 Vue.js 在不同的平台下都会有编译的过程，因此编译的编译函数、配置都会有所不同。编译过程会多次执行，但这两个不同的变量在同一个平台下每一次的编译过程又是相同的，为了不让他们在每次编译过程都通过参数传入，Vue.js 利用了函数柯里化的技巧很好的实现了 `baseOptions` 和 `baseCompile` 的参数保留。